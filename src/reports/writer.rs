...
use crate::reports::{
    CharSet, Config, IndexType, LabelAttach, LabelDisplay, Report, ReportStyle, Show, Span, Write,
};

use super::renderer::Characters;
...
impl<S: Span, K: ReportStyle> Report<S, K> {
    ...
    fn write_for_stream<C: Cache<S::SourceId>, W: Write>(
        ...
        ) in groups.into_iter().enumerate()
        {
            ...
            let line_ref = format_args!("{src_name}:{line_no}:{col_no}");
            writeln!(
                w,
                "{}{}{}{} {} {}",
            ...
            if !self.config.compact {
                if is_final_group {
                    let final_margin =
                        format_args!("{}{}", Show((draw.hbar, line_no_width + 2)), draw.rbot);
...
//     //     let source = format_args!("{}orange", "apple == ".repeat(100));
//     //     let msg = remove_trailing(
//     //         Report::build(ReportKind::Error, 0..0)
//     //             .with_config(no_color_and_ascii())
//     //             .with_message("can't compare apples with oranges")
