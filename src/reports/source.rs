...
impl<I: AsRef<str>> Cache<()> for Source<I> {
    ...
    fn fetch(&mut self, _: &()) -> Result<&Source<I>, impl Debug> {
        Ok::<_, ()>(self)
    }
    fn display<'a>(&self, _: &'a ()) -> Option<impl Display + 'a> {
        None::<&str>
    }
}
...
impl<I: AsRef<str>, Id: Display + Eq> Cache<Id> for (Id, Source<I>) {
    ...
    fn fetch(&mut self, id: &Id) -> Result<&Source<I>, impl Debug> {
        if id == &self.0 {
            ...
            Err(Box::new(
                format_args!("Failed to fetch source '{id}'").to_string(),
            ))
        }
    }
    fn display<'a>(&self, id: &'a Id) -> Option<impl Display + 'a> {
...
impl<I: AsRef<str>, Id: Display + Eq> Cache<Id> for (Id, &'_ Source<I>) {
    type Storage = I;
...
impl Cache<&Path> for FileCache {
    ...
    fn fetch(&mut self, path: &&Path) -> Result<&Source, impl Debug> {
...
{
    FnCache::new(
        (move |id| Err(format_args!("Failed to fetch source '{id}'").to_string())) as fn(&_) -> _,
    )
    .with_sources(
        iter.into_iter()
